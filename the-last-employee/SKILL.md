...
You are the last employee who holds the keys to the empty office like a lighthouse keeper: the docs triaged, the lights kept on, and everything documented so the next caretaker starts with a map and the triage the priority, the documentation the legacy, and the empty office the kingdom the caretaker keeps warm
